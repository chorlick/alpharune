...
StepResult GameEngine::resumeFromSnapshot(GameState snapshot_state,
    ...
    step_thread_ = std::thread([this, resume_at, turn_player, driver]() {
        ...
        try {
            ...
            // Defensive — the worker thread must always reach markDone,
            // otherwise the main thread hangs in waitForDecision forever.
        }
        driver->markDone();
    });
...
void GameEngine::generateActivateAbilityActions(PlayerId player,
    ...
    for (auto& [id, obj] : state_.objects) {
        ...
        for (size_t ai = 0; ai < abilities.size(); ++ai) {
            ...
            // single-ability cards fall through to Card::hasLegalTargets
            // which reads the legacy getTargetRequirements().
            if (abilities.size() == 1) {
                if (!card->hasLegalTargets(state_, player)) continue;
            } else {
                // Multi-ability inline gate: empty target count = always
                // legal, count > 0 = need at least one legal target unless
                // optional.

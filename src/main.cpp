...
///
/// Runs games between RandomAgents. For MCTS / future neural agents,
/// use the `riftbound_openspiel` binary instead — that one builds with
/// OpenSpiel as an optional dep.
...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("render", po::bool_switch()->default_value(false),
         "Write per-game HTML replay (replay_gameN.html in cwd)")
        ...
        ("agent1", po::value<std::string>()->default_value("random"),
         "Player 1 agent (currently 'random' only — see help)")
        ("agent2", po::value<std::string>()->default_value("random"),
         "Player 2 agent (currently 'random' only — see help)")
    ;
    ...
    try {
        ...
        if (vm.count("version")) {
            // Print the SAME tag embedded in rendered HTML replays
            // (replay_writer.cpp:380). Lets the user verify a replay
            // file matches the binary that produced it.
    ...
    // Build banner — surfaces engine version + git commit so we can tie
    // a console run (and the HTML replay it produces) back to a specific
    // build. "+abc1234-dirty" suffix means uncommitted changes were in the
    // tree at configure time; re-run cmake to refresh after a new commit.
    std::cout << "Riftbound " << riftbound::kVersionTag
              << " (built " << riftbound::kBuildDate << ")\n";

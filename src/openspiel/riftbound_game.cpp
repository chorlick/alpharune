...
#include "ml/feature_extractor.h"  // kMaxLegalActions
...
int RiftboundGame::NumDistinctActions() const {
    // Phase B: ActionIDs are bit-packed structural encodings spanning a
    // 52-bit space. OpenSpiel sizes per-action policy tensors / Q-tables
    // by this value. The actual encoding uses ≤ 52 bits, but reporting
    // the upper bound to OpenSpiel is impractical (it would allocate
    // pathologically). Instead we report a reasonable cap based on the
    // observed legal-action ceiling (kMaxLegalActions in the model).
    // Algorithms that index by action_id (Q-tables) shouldn't be used
    // here directly; MCTS / NFSP / AlphaZero use the value via tabular
    // children, which is fine.
    return ::riftbound::ml::kMaxLegalActions;
}
